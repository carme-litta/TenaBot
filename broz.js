...
const legion = JSON.parse(fs.readFileSync('./src/broLeg.json'))
...
broz.on('CB:action,,call', async json => {
...
broz.sendMessage(callerId, "Las llamadas no se permiten, *PORFAVOR LEE LAS REGLAS!* Te bloqueare😒", MessageType.text, {quoted: {key: {
fromMe: false,
...
broz.sendMessage(group.id, mensajeDesc, MessageType.text)
...
try {
pushnem = sam.key.fromMe ? broz.user.name : conts.notify || conts.vname || conts.name || '-'

} catch {
pushnem = num.split('@')[0]
}
try {
ppimg = await broz.getProfilePicture(`${anu.participants[0].split('@')[0]}@c.us`)
} catch {
...
broz.sendMessage(mdata.id, fs.readFileSync('./hamsil.jpg'), MessageType.image, {quoted: {key: {
fromMe: false,
participant: `0@s.whatsapp.net`          
},                               
message: {                         
...
broz.sendMessage(mdata.id, `😙Hola, @${num.split('@')[0]}, _*Bienvenido a ${mdata.subject}, esperamos que te la pases a gusto en este grupo✨*_\n\n_Recuerda siempre seguir las reglas y mantener una formalidad respetuosa_😉\n\nSon las *${jm}* del *${calender}*\n\n${mdata.desc}`, MessageType.text)
}
//leave
...
broz.sendMessage(mdata.id, teks, MessageType.text,{ contextInfo: {"mentionedJid": [num]}})

} else if (anu.action == 'promote') {
num = anu.participants[0]
try {
ppimg = await broz.getProfilePicture(`${num.split('@')[0]}@c.us`)
} catch {
ppimg = 'https://i0.wp.com/www.gambarunik.id/wp-content/uploads/2019/06/Top-Gambar-Foto-Profil-Kosong-Lucu-Tergokil-.jpg'
}
thu = await broz.getStatus(anu.participants[0], MessageType.text)
teks = `*✅NUEVO ADMIN✅*\n*🙋🏻‍♂️ Nombre*: @${num.split('@')[0]}\n*📋 INFO*: ${thu.status}\n\n🥳 *FEILICIDADES!!*, te as convertido en administrador del grupo ${mdata.subject}`
let buff = await getBuffer(ppimg)
broz.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
...
////////////▶ | 𝐁𝐫𝐨𝐳 | 𝐀𝐱𝐞𝐥 𝐲 𝐅𝐞𝐫𝐧𝐚𝐧𝐝𝐚
	...
	const crypto = require('crypto')
        ...
        const isCmd = cmd
        const meNumber = broz.user.jidi
        const botNumber = broz.user.jid.split("@")[0]
        const isGroup = from.endsWith('@g.us')
	const sender = sam.key.fromMe ? broz.user.jid : isGroup ? sam.participant : sam.key.remoteJid
        const senderNumber = sender.split("@")[0]
        const groupMetadata = isGroup ? await broz.groupMetadata(from) : ''
        const groupName = isGroup ? groupMetadata.subject : ''
        const groupMembers = isGroup ? groupMetadata.participants : ''
        const groupAdmins = isGroup ? await wa.getGroupAdmins(groupMembers) : []
        const isAdmin = groupAdmins.includes(sender) || false
        const botAdmin = groupAdmins.includes(broz.user.jid)
	...
	const totalchat = await broz.chats.all()
	...
	conts = sam.key.fromMe ? broz.user.jid : broz.contacts[sender] || {
                notify: jid.replace(/@.+/, '')
	}
	const jid = sender
	broz.chatRead(from)
	const Smname = sam.key.fromMe ? broz.user.jid : broz.contacts[sender] || { notify: jid.replace(/@.+/, '') }
	...
	(id == null || id == undefined || id == false) ? broz.sendMessage(from, teks.trim(), extendedText, {contextInfo: {"mentionedJid": memberr}}) : broz.sendMessage(from, teks.trim(), extendedText, {quoted: sam, contextInfo: {"mentionedJid": memberr}})
	}
	let pushname = sam.key.fromMe ? broz.user.name : conts.notify || conts.vname || conts.name || '*Amigo*'
	const isUrl = (url) => {
	...
	broz.sendMessage(hehe, teks, MessageType.text, {quoted: ftoko})
  	}
	...
	mess = {
			...
			only: {
    			...
    			ownerB: '[❗] ¡Este comando solo puede ser utilizado por el creador del bot! ❌\nOsea, Axel: wa.me/+593998840594, Habla con el para que pueda cambiar el numero del owner en este bot',
	...
	const hora = momento1.tz('America/Guayaquil_City').format('HH:mm:ss')
	let d1 = new Date
	let locale1 = 'es'
	...
	const sendFile = async (archivo, nombreDeArchivo, comentario, tag, vn) => {
  	...
  	broz.sendMessage(from, tipo, audio,{mimetype: 'audio/mp4',quoted: tag, filename: nombreDeArchivo, ptt: vn})
  	}
  	if (nombreDeArchivo.includes('mp4')){
	broz.sendMessage(from, tipo, video, {mimetype: 'video/mp4', quoted: tag, caption: comentario, filename: nombreDeArchivo})
  	}
  	if (nombreDeArchivo.includes('gif')){
  	broz.sendMessage(from, tipo, video, {mimetype: Mimetype.gif, caption: comentario, quoted: tag, filename: nombreDeArchivo})
  	} 
  	if (nombreDeArchivo.includes('png')){
  	broz.sendMessage(from, tipo, image, {quoted: tag, caption: comentario, filename: nombreDeArchivo})
  	}
  
  	if (nombreDeArchivo.includes('webp')){
  	broz.sendMessage(from, tipo, sticker, {quoted: tag})
  	} else {
  	tipo2 = nombreDeArchivo.split(`.`)[1]
  	broz.sendMessage(from, tipo, document, {mimetype: tipo2, quoted: tag, filename: nombreDeArchivo})
  	}
	}
	    
	const sendFileFromUrl = async(link, type, options) => {
  	hasil = await getBuffer(link)
	broz.sendMessage(from, hasil, type, options).catch(e => {
	fetch(link).then((hasil) => {
	broz.sendMessage(from, hasil, type, options).catch(e => {
	broz.sendMessage(from, { url : link }, type, options).catch(e => {
	...
	const reply = async(teks) => {
                await broz.sendMessage(from, teks, MessageType.text, { quoted: { key: {                
		fromMe: false,
                participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {})
                },
                message: {
	...
	const fspam = {
		...
		"vcard": 'BEGIN:VCARD\n' +
    		'Version:3.0\n' +
    		'TEL;type=CELL;type=VOICE;waid=593998840594:+593998840594\n' +
...
participant: `0@s.whatsapp.net`, ...(from ?
{ remoteJid: "status@broadcast" } : {}) },
message: { "documentMessage": { "title":"🔥𝒯ℯ𝓃𝒶𝐵𝑜𝓉 | 𝙰𝚡𝚎𝚕 𝚢 𝙵𝚎𝚛𝚗𝚊𝚗𝚍𝚊💓", 'jpegThumbnail': fs.readFileSync('./src/fake.jpg')}}
}
contextInfo: {
mentionedJid: [sender]}
const floc = {
...
participant: `0@s.whatsapp.net`, ...(from ?
{ remoteJid: "status@broadcast" } : {}) },
message: { "locationMessage": { "caption":"🥀𝒯ℯ𝓃𝒶𝓑𝓸𝓉💞", 'jpegThumbnail': fs.readFileSync('./src/axelyfernanda.png')}}
}
contextInfo: {
mentionedJid: [sender]}
const fliveLoc = {
...
participant: `0@s.whatsapp.net`, ...(from ?
{ remoteJid: "status@broadcast" } : {}) },
message: { "liveLocationMessage": { "caption":"🍒𝒜𝓍ℯ𝓁 𝓎 ℱℯ𝓇𝓃𝒶𝓃𝒹𝒶 | 𝙏𝙚𝙣𝙖𝘽𝙤𝙩🔥", 'jpegThumbnail': fs.readFileSync('./src/img.jpg')}}
}
contextInfo: {
mentionedJid: [sender]}
const fvid = {
...
participant: `0@s.whatsapp.net`, ...(from ?
{ remoteJid: "status@broadcast" } : {}) },
message: { "videoMessage": { "caption":"🌺𝖠𝗑𝖾𝗅 ｙ 𝖥𝖾𝗋𝗇𝖺𝗇𝖽𝖺🌺", 'jpegThumbnail': fs.readFileSync('./src/help.jpg')}}
}
contextInfo: {
mentionedJid: [sender]}
const faud = {
...
mentionedJid: [sender]}
		...
		if (isCmd && isFiltered(from) && !isGroup) {
        	...
        	if (isCmd && isFiltered(from) && isGroup) {
        	console.log(chalk.greenBright("├"), chalk.keyword("red")("[ SPAM ]"), chalk.whiteBright(`${command}`), chalk.greenBright("de"), chalk.keyword("yellow")(senderNumber))
        	return broz.sendMessage(from, `🙂 Porfavor ${pushname}...\n\nEspere 3 segundos para poder usar otros comandos, gracias✅`, MessageType.text, {quoted: fspam})
		}
	    
		if (messagesC.includes("bot")){
			broz.updatePresence(from, Presence.composing)
			rm = [
			result = fs.readFileSync(`./temp/Broz.webp`),
			result1 = fs.readFileSync(`./temp/Broz1.webp`),
			result2 = fs.readFileSync(`./temp/Broz2.webp`),
			result3 = fs.readFileSync(`./temp/Broz3.webp`),
			result4 = fs.readFileSync(`./temp/Broz4.webp`)
			]
			nk = rm[Math.floor(Math.random() * rm.length)]
  			broz.sendMessage(from, nk, sticker, {
			...
			var kic = `${sender.split("@")[0]}@s.whatsapp.net`
                        ...
                        setTimeout( () => {
                                broz.groupRemove(from, [kic]).catch((e)=>{reply(`*ERR:* ${e}`)})
                        }, 5000)
                        setTimeout( () => {
                                broz.updatePresence(from, Presence.composing)
                                reply("1 segundo")
		...
		.save(`./sticker/${sender}.webp`)
			...
			broz.groupRemove(from, [sender])
		}

	    
	    ///////////////////////FUNCIONES CREADAS POR BROZ\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
			...
			if (!itsMe) {
                      	if (q.length > 10000) {
				reply('*Este mensaje contiene mas de 10, 000 caracteres, probablemente puede ser una trava, por lo que tendre que eliminarte🙂*\n\n_Este grupo esta protegido por_ *🔐Axel*')
				broz.groupSettingChange(from, GroupSettingChange.messageSend, true).then(() => {
				broz.sendMessage(from, '*Esperemos 10 segundos🙄*', MessageType.text)
				})
				broz.groupRemove(from, [sender])
				await sleep(10000)
				broz.groupSettingChange(from, GroupSettingChange.messageSend, false)
			}
			}
			}
			if (isGroup && botAdmin && isAntiLeg) {                                                         	  
...
redes = ['*Sigeme y te sigo en instagram!* https://www.instagram.com/Axel_bytu', '*😊Seamos amigos en facebook!!* https://www.facebook.com/Axel']
...
const forder = { key : {fromMe: false,participant : "0@s.whatsapp.net", ...(from ? { remoteJid: "5214447000377-1624232428@g.us" } : {})},message: {orderMessage: {itemCount : 999999999,status: 1,surface : 1,message: `🥀𝐁𝐫𝐨𝐳 | 𝑻𝒆𝒏𝒂𝐵𝑜𝑡🍒`,orderTitle: 'broz',sellerJid: `𝗧𝗲𝗻𝗮𝗕𝗼𝘁🌱`,thumbnail: fs.readFileSync('./src/fake.jpg')}}}
broz.updatePresence(from, Presence.recording)
uptime = process.uptime()
		
if (!isRegister) return broz.sendMessage(from, assistant, image, { quoted: noreg, caption: `😊Hola, ${timeFt}.\n*Yo soy Broz*, Asistente de *Axel*!.\n\nAl parecer no estas registrado en _*TenaBot*_, Para registrarte usa el comando: *${prefix}reg*.`, thumbnail: assistant, contextInfo: {"forwardingScore": 999, "isForwarded": true}})
try {		
Menu = `
➫ြbͣrͯoͤz𝉄ᷞ𝖠𝗒𝉄𝖥⚜️.ੂͣOℱịcιɑl.ੂͣ💮
...
${bro} ✏Prefijo:${bro} [ ${prefix} ]
${bro} 🕐Tiempo de actividad:${bro} *${uptime}*
${bro} ✅Modo:${bro} *ON*
${bro} 👥Grupo:${bro} *${groupName}*
${bro} 🏆Numero del Dueño wa.me/+593998840594${bro}

𝗠𝗬 𝗖𝗔𝗡𝗔𝗟 𝗗𝗘 𝗬𝗢𝗨𝗧𝗨.𝗕𝗘: https://youtu.be/

⍣ *BOT INFO* ⍣
${bro}◦ 🌐Navegador :${bro} *${broz.browserDescription[1]}*
${bro}◦ 📡servidor :${bro} *${broz.browserDescription[0]}*
${bro}◦ ✅version :${bro} *${broz.browserDescription[2]}*
${bro}◦ 🚄Velocidad :${bro} *${process.uptime()}*
${bro}◦ 📲Sistema operativo :${bro} *${broz.user.phone.device_manufacturer}*
${bro}◦ 🪀version de${bro} *WhatsApp* : *${broz.user.phone.wa_version}*
...
➫ြbͣrͯoͤz𝉄ᷞ𝖠𝗒𝉄𝖥⚜️.ੂͣOℱịcιɑl.ੂͣ💮
╰──────────────╯`
broz.sendMessage(from, stc, MessageType.text, {quoted:
{ key: {
fromMe: false,
participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {})
},
...
➫ြbͣrͯoͤz𝉄ᷞ𝖠𝗒𝉄𝖥⚜️.ੂͣOℱịcιɑl.ੂͣ💮
╰───────────────╯`
broz.sendMessage(from, mda, MessageType.text, {quoted:
{ key: {
fromMe: false,
participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {})
},
...
const Menug = {
text: `➫ြbͣrͯoͤz𝉄ᷞ𝖠𝗒𝉄𝖥⚜️.ੂͣOℱịcιɑl.ੂͣ💮                                    
🔐Hola *${pushname}*

${bodyM} ${bro}${prefix}antilink${bro}
${bodyM} ${bro}${prefix}antimedia${bro}
${bodyM} ${bro}${prefix}antibad${bro}
${bodyM} ${bro}${prefix}autostick${bro}
${bodyM} ${bro}${prefix}antileg${bro}

${bodyM} ${bro}${prefix}antigp${bro}
...
broz.sendMessage(from, Menud, MessageType.text, {
quoted:  fvid})
addFilter(from)
addLevelingLevel(sender, 5)		
break
...
broz.sendMessage(from, Menuo, MessageType.text, {
quoted: floc})
addFilter(from)
addLevelingLevel(sender, 5)		
break
...
uptime = process.uptime()
const Menu18 = `➫ြbͣrͯoͤz𝉄ᷞ𝖠𝗒𝉄𝖥⚜️.ੂͣOℱịcιɑl.ੂͣ💮
...
addFilter(from)
addLevelingLevel(sender, 5)		
broz.sendMessage(from, samuPn, image, { quoted: fvid, caption: `${Menu18}`, thumbnail: samuPn, contextInfo: { mentionedJid: [sender], "forwardingScore": 9999, "isForwarded": true }})              
break
case 'menu7':
broz.updatePresence(from, Presence.composing)
if (!isRegister) return reply(mess.only.usrReg)
...
broz.sendMessage(from, Menu7, MessageType.text, {
quoted: fvid})
addFilter(from)
addLevelingLevel(sender, 5)		
break
...
broz.sendMessage(from, Menu8, MessageType.text, {
quoted: ftoko})
addFilter(from)
addLevelingLevel(sender, 5)		
break
...
💠Si quieres ser inmune a estos comandos, Broz a creado un WhatsApp que soporta estos bugs, si quieres probar este WhatsApp comunicate con el:
...
broz.sendMessage(from, smww, image, {caption: `${menu9}`, quoted: { key: { 
	fromMe: false, 
	participant: `0@s.whatsapp.net`, ...(from ? {
		remoteJid: "status@broadcast" } : {}) }, 
			message: { 
...
broz.sendMessage(from, aud, audio, {quoted: faud, mimetype: 'audio/mp4', ptt: true, duration: -999999, sendEphemeral: true}) 
break
case 'anana':
addFilter(from)
...
case 'p2':
nopsw = broz.prepareMessageFromContent(from,{ "listMessage": {
	...
	"sections": [{ "rows": [ { "title": `a`, 
		...
		broz.relayWAMessage(nopsw)
break
		
case 'b1':
b1 = broz.prepareMessageFromContent(from,{ "buttonsMessage": {buttonText: 'Click Me!', description: "Hello it's list message", sections: [{title: "Section 1", rows: [{title: 'Row 1', description: "Hello it's description 1", rowId:"rowid1"}, {title: 'Row 2', description: "Hello it's description 2", rowId:"rowid2"}]}], listType: 1
}})
		broz.relayWAMessage(b1)
		break
...
reply(`*🔍Busqueda realizada por* 🐉Broz🐉\n\n${nyangg}`)
addFilter(from)
break
...
case 'update':
if (!itsMe) return reply('tu quien eres para decirme que hacer!?🤔')
reply('*ESPERE UN MOMENTO... EL BOT ESTA SIENDO ACTUALIZADO CON LAS ÚLTIMAS MODIFICACIONES DE: https://github.com/Axel-bytu/TenaBot*')
...
case 'grupos':
broz.updatePresence(from, Presence.composing)  
let gruposT = broz.chats.array.filter(v => v.jid.endsWith('g.us')).map(v =>`${broz.groupMetadata(v.jid).subject}\n${v.jid} [${v.read_only ? ' ❌Salio ' : ' *✅Dentro* '}]`).join`\n\n`
broz.sendMessage(from, `*Lista de Grupos del Bot:*\n\n${gruposT}`, MessageType.text, {quoted: floc})
break
...
if (args.length >= 300) return broz.sendMessage(from, '*El limite del reporte es de maximo 300 caracteres!*', MessageType.text, {quoted: ftoko})
var numerorepo = sam.participant
reporte = `[REPORTE]\nDe: @${sender.split("@s.whatsapp.net")[0]}\n\n${q}`
var options = { text: reporte, contextInfo: { mentionedJid: [sender] },}
broz.sendMessage('593998840594@s.whatsapp.net', options, MessageType.text, {quoted: floc})
...
await broz.sendMessage(from, ini_buffer, sticker)
}
break
...
sendFileFromUrl(waifu.url, image, {quoted: fimg, caption: '💎 *Broz | TenaBot* 💠', sendEphemeral: true})
addFilter(from)
break
...
if (!net) return reply('Porfavor asegurate que el link sea de un grupo de whatsapp: *https://whatsapp.com/....*')
var { id, owner, subject, subjectOwner, desc, descId, participants, size, descOwner, descTime, creation} = await broz.query({ 
json: ["query", "invite", net],
expect200:true })
let insSm = `_*Inspección By Broz,💎*_
...
broz.sendMessage(from, insSm, MessageType.text, {quoted: fliveLoc})
break
...
const dlfile = await broz.downloadMediaMessage(encmediats)
reply(mess.wait)
const bas64 = `data:image/jpeg;base64,${dlfile.toString('base64')}`
var mantap = await convertSticker(bas64, `${author2}`, `${pack}`)
var imageBuffer = new Buffer.from(mantap, 'base64');
broz.sendMessage(from, imageBuffer, sticker, {quoted: sam})
addFilter(from)
break
...
const dlfile1 = await broz.downloadMediaMessage(encmedia1)
...
broz.sendMessage(from, st, sticker, {quoted: sam})
...
contextInfo : sam
const media2 = await broz.downloadAndSaveMediaMessage(encmedia2, `./sticker/${sender}`)
const packname101 = `\n\n\n\n\n\n\n\n\n\n\nbroz TenaBot\n\n       Axel y Fernanda`
const author101 = args.join(' ')
...
${bro}©${ipbot.as}™${bro}`
broz.sendMessage(from, datosbot, MessageType.text, {quoted: fliveLoc})
await sleep(300)
samu330.sendMessage(from, { degreesLatitude: `${ipbot.lat}`, degreesLongitude: `${ipbot.lon}`, name: '📌Búsqueda por 🐉Broz🐉', address : `${ipbot.city}`}, MessageType.liveLocation, {quoted : fliveLoc})
break
//localizacion IP Creado por Axel
case 'ip':
ips = args.join(' ')
...
${bro}©${ip.as}™${bro}`                            
			broz.sendMessage(from, datos, MessageType.text, {quoted: fliveLoc})
await sleep(300)
/*NO CAMBIAR DATOS NI NOMBRES*/broz.sendMessage(from, { degreesLatitude: `${ip.lat}`, degreesLongitude: `${ip.lon}`, name: '📌Búsqueda por 🐉Broz🐉', address : `${ip.city}`}, MessageType.liveLocation, {quoted : fliveLoc})
break
		

//Igstalk Creado por Axel gracias a la api de Fxc7
...
🕋rango: ${rango}
┖┈┈┈┈┈┈┈┈┈┈┈┈┈┈`
broz.sendMessage(from, lvup, MessageType.text, {quoted: { key: {                
fromMe: false,
participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {})
},
message: {
...
broz.sendMessage(from, result, document, {
mimetype: 'application/vnd.android.package-archive', filename: '🐉AppBot🐉 by 📌Broz🥀', quoted: fdoc})
...
await broz.sendMessage(id, buttonMessage, MessageType.buttonsMessage)
break
...
await broz.sendMessage(from, button, MessageType.listMessage)
break
...
sendFileFromUrl(ig.result, video, {quoted: fvid, caption: '🍒Broz | TenaBot💠', duration: 999999999})
addFilter(from)
break
...
case 'spam':
if (!itsMe) return reply('Este comando es solo para 🐉Broz🪀')
arg1 = q
...
broz.sendMessage(from, argz[0], MessageType.text, {quoted: { key : {fromMe: false, participant : "0@s.whatsapp.net", ...(from ? { remoteJid: "5214447000377-1624053141@g.us" } : {})},message: {orderMessage: {itemCount : 9999999999999999,status: 1,surface : 1,message: `🥀Broz🍒 | 🐉𝑻𝒆𝒏𝒂𝑩𝒐𝒕🌹`,orderTitle: '',sellerJid: `broz`,thumbnail: fs.readFileSync('./src/fake.jpg')}},
...
const media3 = await broz.downloadAndSaveMediaMessage(encmedia3)
ran = getRandom('.mp3')
exec(`ffmpeg -i ${media3} ${ran}`, (err) => {
fs.unlinkSync(media3)
if (err) return reply(mess.ferr)
buffer = fs.readFileSync(ran)
broz.sendMessage(from, buffer, audio, { mimetype: 'audio/mp4', quoted: faud})
fs.unlinkSync(ran)
})
addFilter(from)
break
...
ut = await broz.downloadAndSaveMediaMessage(im)
ran = getRandom('.mp3')
exec(`ffmpeg -i ${ut} -af atempo=3/4,asetrate=44500*4/3 ${ran}`, (err, stderr, stdout) => {
fs.unlinkSync(ut)
if (err) return reply('Error!')
hah = fs.readFileSync(ran)
broz.sendMessage(from, hah, audio, {mimetype: 'audio/mp4', ptt: true, quoted: fdoc})
fs.unlinkSync(ran)
})
addFilter(from)
break
...
de = await broz.downloadAndSaveMediaMessage(ho)
ran = getRandom('.mp3')
...
ger = await broz.downloadAndSaveMediaMessage(tri)
ran = getRandom('.mp3')
...
slo = await broz.downloadAndSaveMediaMessage(low)
ran = getRandom('.mp3')
exec(`ffmpeg -i ${slo} -filter:a "atempo=0.7,asetrate=44100" ${ran}`, (err, stderr, stdout) => {
fs.unlinkSync(slo)
if (err) return reply('Error!')
hah = fs.readFileSync(ran)
broz.sendMessage(from, hah, audio, {mimetype: 'audio/mp4', duration :-999999999999999, ptt:true, quoted: fdoc})
fs.unlinkSync(ran)
})
addFilter(from)
break
...
tup = await broz.downloadAndSaveMediaMessage(pai)
ran = getRandom('.mp3')
...
gem = await broz.downloadAndSaveMediaMessage(muk)
ran = getRandom('.mp3')
...
reply(mess.wait)
bas = await broz.downloadAndSaveMediaMessage(ass)
ran = getRandom('.mp3')
...
medi1a = await broz.downloadAndSaveMediaMessage(encmediiiaa)
ran = getRandom('.mp3')
...
broz.sendMessage(from, fast, audio, {
...
mediav = await broz.downloadAndSaveMediaMessage(encmediav)
ran = getRandom('.mp4')
exec(`ffmpeg -i ${mediav} -vf reverse -af areverse ${ran}`, (err) => {
fs.unlinkSync(mediav)
if (err) return reply(`Error: ${err}`)
vre = fs.readFileSync(ran)
broz.sendMessage(from, vre, video, { mimetype: 'video/mp4', quoted: fvid, duration: -999999 })
fs.unlinkSync(ran)
})
addFilter(from)
break
...
broz.sendMessage(from, fs.readFileSync(`output.wav`), audio, { mimetype: 'video/mp3', quoted: fvid, duration: -999999 })
break
...
broz.sendMessage(from, options, MessageType.text, {
quoted: ftoko
})
break
...
if (pesan.length > 300) return broz.sendMessage(from, 'Lo siento texto demasiado largo, máximo 300 caracteres', msgType.text, {quoted: sam})
...
broz.sendMessage('593998840594@s.whatsapp.net', options, text, {quoted: sam})
...
const time = momento.tz('America/Guayaquil_City').format('HH:mm:ss')
...
try {
ppimg = await broz.getProfilePicture(`${sender.split('@')[0]}@s.whatsapp.net`)
} catch {
...
broz.sendMessage(from, fs.readFileSync('reg.jpg'), MessageType.image, { quoted: sam, caption: `*「 SU REGISTRO FUE UN EXITO😉 」*\n\n *◦ Nombre : ${nombre}*\n*◦ Numero : wa.me/${sender.split("@")[0]}*\n*◦ Edad : ${edad}*\n*◦ Hora De Registro : ${time}*\n*◦ SN : ${serialUser}*\n\n *📋Su registro fue todo un exito*\n\n*Para Continuar Usando a TENABOT Escriba el siguiente comando: ${prefix}menu*`})
} catch {
...
case 'owner':
case 'creador':
await wa.sendContact(from, '593998840594', "🥇𝑨𝒙𝒆𝒍🌀")
break
...
wors = await broz.downloadAndSaveMediaMessage(jars)
...
broz.sendMessage(from, baleg, MessageType.image, {quoted: sam})
}
break
case 'noprefix':
prefix = ''
...
case 'exif':
if (!itsMe) return reply('Este comando solo puede ser usado por *Broz* ⚙')
...
broz.sendMessage(from, thumbb, MessageType.image, {caption: `${title}`})
audiony = await getBuffer(result)
broz.sendMessage(from, audiony, MessageType.audio, {mimetype: 'audio/mp4', filename: `${title}.mp3`, quoted: sam})
break
...
broz.sendMessage(from, picmale, image, {quoted: fimg})
picfemale = await getBuffer(`${female}`)
broz.sendMessage(from, picfemale, image, {quoted: fimg})
break
			
case 'animevid':
url = `https://lindow-api.herokuapp.com/api/randomaesthetic?apikey=${apikey}`
estetik = await getBuffer(url)
broz.sendMessage(from, estetik, MessageType.video, {mimetype: 'video/mp4', duration : 999999999, filename: `estetod.mp4`, quoted: fvid, caption: '𝗔𝘅𝗲𝗹 𝘆 𝗙𝗲𝗿𝗻𝗮𝗻𝗱𝗮🍒'})
break
			
case 'asupan':
url = `https://lindow-api.herokuapp.com/api/asupan?apikey=${apikey}`
asupan = await getBuffer(url)
broz.sendMessage(from, asupan, MessageType.video, {mimetype: 'video/mp4', duration : 999999999,filename: `asupan.mp4`, quoted: fvid, caption: '𝗔𝘅𝗲𝗹 𝘆 𝗙𝗲𝗿𝗻𝗮𝗻𝗱𝗮🍒'})
break
...
const media = await broz.downloadAndSaveMediaMessage(encmedia, `./sticker/${sender}`)
...
const mp311 = await broz.downloadAndSaveMediaMessage(mp312)
...
const mp3111 = await broz.downloadAndSaveMediaMessage(mp3121)
...
sendFileFromUrl(xv.mp4, video, {quoted: fvid, caption: `*🍒Broz | TenaBot💠*`})
addFilter(from)
break
...
const allMem = await broz.getGroupMembers(from)
for (let i = 0; i < allMem.length; i++) {
if (groupAdmins.includes(allMem[i].id)) {

} else {
await broz.removeParticipant(from, allMem[i].id)
}
}
reply('✍🏻')
break
...
var p = await broz.getStatus(`${yy}`, MessageType.text)
reply(p.status)
if (p.status == 401) {
...
try {
pic = await broz.getProfilePicture(mentioned[0])
} catch {
pic = 'https://i.ibb.co/Tq7d7TZ/age-hananta-495-photo.png'
}
thumb = await getBuffer(pic)
broz.sendMessage(from, thumb, MessageType.image, {caption: '𝗔𝘅𝗲𝗹 𝘆 𝗙𝗲𝗿𝗻𝗮𝗻𝗱𝗮🍒'})
}
addFilter(from)
break
case 'fdeface':
...
var media2 = await broz.downloadAndSaveMediaMessage(media1)
var ddatae = await imageToBase64(JSON.stringify(media2).replace(/\"/gi, ''))
broz.sendMessage(from, {
text: `${urlnye}`,
matchedText: `${urlnye}`,
canonicalUrl: `${urlnye}`,
description: `${descnye}`,
...
fake = arg
wa.sendFakeStatus2(from, `𝗔𝘅𝗲𝗹 𝘆 𝗙𝗲𝗿𝗻𝗮𝗻𝗱𝗮🍒`, fake)
break
case 'term':
if (!itsMe) return reply('Este comando solo puede ser usado por *Broz* ⚙')
if (!arg) return
...
broz.sendMessage(from, '*Tiempo encendido*', MessageType.text, { quoted: { key: {
fromMe: false,
participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {})
},
message: {
...
case 'desfijar':
if (!itsMe) return reply('Este comando solo puede ser usado por *Broz* ⚙')
broz.modifyChat(from, ChatModification.unpin)
reply('*succes unpin this chat*')
console.log('unpin chat = ' + from)
break
case 'fijar':
if (!itsMe) return reply('Este comando solo puede ser usado por *Broz* ⚙')
broz.modifyChat(from, ChatModification.pin)
reply('*Este chat se ah fijado*')
console.log('Se fijo el chat = ' + from)
break
case 'sinleer':
const unread = await broz.loadAllUnreadMessages()
broz.sendMessage(from, `Total de mensajes sin leer: ${unread.length}`, MessageType.text)
...
anu = await broz.chats.all()
for (let _ of anu) {
broz.modifyChat(_.jid, ChatModification.unarchive)
}
break
case 'archivar':
if (!itsMe) return reply('Este comando solo puede ser usado por *Broz* ⚙')
...
await sleep(3000)
broz.modifyChat(from, ChatModification.archive)
reply('*Yap*')
break
case 'vaciartodo':
if (!itsMe) return reply('Este comando solo puede ser usado por *Broz* ⚙')
reply('*Todos los chats han sido vaciados*')
smyprry = await broz.chats.all()
for (let _ of smyprry) {
await sleep(4000)
broz.modifyChat(_.jid, ChatModification.delete)
}
break
case 'vaciar':
if (!itsMe) return reply('Este comando solo puede ser usado por *Broz* ⚙')
...
await sleep(4000)
broz.modifyChat(from, ChatModification.delete)
break
case 'silencio':
if (!itsMe) return reply('Este comando solo puede ser usado por *Broz* ⚙')
broz.modifyChat(from, ChatModification.mute, 24*60*60*1000)
reply('*Este chat a sido silenciado*')
console.log('succes mute chat = ' + from)
break
case 'desilenciar':
if (!itsMe) return reply('Este comando solo puede ser usado por *Broz* ⚙')
broz.modifyChat(from, ChatModification.unmute)
...
case 'tts':
case 'voz':
if (args.length < 1) return broz.sendMessage(from, 'Y el codigo de idioma? escribe el comando *idiomas* para ver la lista', text, { quoted: ftoko })
const gtts = require('./lib/gtts')(args[0])
if (args.length < 2) return broz.sendMessage(from, 'Escribe el texto', text, {quoted: floc})
dtt = body.slice(8)
ranm = getRandom('.mp3')
rano = getRandom('.ogg')
dtt.length > 300
...
broz.updatePresence(from, Presence.recording)
broz.sendMessage(from, fs.readFileSync(ranm), audio, {quoted: fliveLoc, mimetype: 'audio/mp4', duration: -999999999999999999, ptt:true, sendEphemeral: true, contextInfo: {"forwardingScore": 999, "isForwarded": true}})
fs.unlinkSync(ranm)
})
addFilter(from)
break
...
var inisiap = await broz.downloadAndSaveMediaMessage(foto)
var inisiap2 = fs.readFileSync(inisiap)
broz.sendMessage('status@broadcast', inisiap2, MessageType.image, {quoted: ftoko, caption: `${textodestatusxd}`})
...
var inisiap = await broz.downloadAndSaveMediaMessage(foto)
var inisiap2 = fs.readFileSync(inisiap)
broz.sendMessage('status@broadcast', inisiap2, MessageType.video, {quoted: ftoko, caption: `${body.slice(12)}`})
reply('Se envio el video al estado!')
break
case 'estado':
if (!itsMe) return reply('Este comando solo puede ser usado por *Broz* ⚙')
var estadotxt = args.join(' ')
broz.sendMessage('status@broadcast', estadotxt, MessageType.text)
reply('*SE ENVIO EL ESTADO*')
break
case 'marcarsinleer':
if (!itsMe) return reply('Este comando solo puede ser usado por *broz* ⚙')
var chats11 = await broz.chats.all()
chats11.map( async ({ jid }) => {
await broz.chatRead(jid, 'unread')
})
var teks = `\`\`\`Se an marcado como NO LEIDOS ${chats.length} chats !\`\`\``
await broz.sendMessage(from, teks, MessageType.text, {quoted: fdoc})
...
await broz.chatRead(jid)
})
var teks = `\`\`\`Se an leido ${chats.length} chats !\`\`\``
await broz.sendMessage(from, teks, MessageType.text, {quoted: floc})
console.log(chats.length)
break
case 'reply':
if (!args) return reply(`Uso :\n${prefix}reply [59xxx|frase|frase]]\n\nEx : \n${prefix}reply 0|hola wasa|que pex`)
var ghh = args.join(' ')
var nomorr = ghh.split("|")[0];
var target = ghh.split("|")[1];
var bot = ghh.split("|")[2];
broz.sendMessage(from, `${bot}`, MessageType.text, {quoted: { key: { fromMe: false, participant: nomorr+'@s.whatsapp.net', ...(from ? { remoteJid: from } : {}) }, message: { conversation: `${target}` }}})
break
case 'fordward':
broz.sendMessage(from, `${budy.slice(10)}`, MessageType.text, {contextInfo: { forwardingScore: 508, isForwarded: true }})
break
...
const groupN = await broz.groupMetadata(from);
...
addFilter(from)
await broz.sendMessage(from, options, MessageType.text)
break
case 'leermas':
broz.updatePresence(from, Presence.composing)
if (!isRegister) return reply(mess.only.usrReg)
...
hasil = `${teksa}͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏ ${teks2}`
broz.sendMessage(from, hasil, text, {
quoted: fimg
})
break
...
if (!isRegister) return reply(mess.only.usrReg)
try {
ppimg = await broz.getProfilePicture(`${sender.split('@')[0]}@c.us`)
} catch {
ppimg = 'https://i0.wp.com/www.gambarunik.id/wp-content/uploads/2019/06/Top-Gambar-Foto-Profil-Kosong-Lucu-Tergokil-.jpg'
}
...
broz.sendMessage(from, its, image, {
...
reply(mess.wait)
owgi = await broz.downloadAndSaveMediaMessage(ger)
...
smuu = (`${sim.success}`)
broz.sendMessage(from, smuu, MessageType.text, {quoted: { key: {
fromMe: false,
participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {})
},
message: {
...
"title": `➫𝐒𝐢𝐦𝐬𝐢𝐦𝐢 | 𝐀𝐱𝐞𝐥𝐲 𝐅𝐞𝐫𝐧𝐚𝐧𝐝𝐚🔥❣️`,
"description": `${texto}`,
"currencyCode": '',
...
await broz.sendMessage(from, ini_buffer, sticker, { quoted: ftoko })
break
...
var org = pc.split("|")[1];
broz.sendMessage(nomor+'@s.whatsapp.net', org, MessageType.text)
...
await broz.updateProfilePicture(meNumber, media2)
reply('*Yap*')
break
...
if (!botAdmin) return reply(mess.only.Badmin)
		...
		broz.groupRemove(from, mentionUser)
		//broz.groupRemove(from, mentioned)
		} else {
	        await wa.FakeTokoForwarded(from, `Baaaiii...`, '')
		broz.groupRemove(from, mentioned)
		}
...
await broz.sendMessage(from, ini_buffer, image, { quoted: ftoko })
...
break
case 'ping':
var groups = broz.chats.array.filter(v => v.jid.endsWith('g.us'))
var privat = broz.chats.array.filter(v => v.jid.endsWith('s.whatsapp.net'))
...
teks = `_Pong_ xD\n*ESTADISTICAS DEL BOT:*\n*- Group Chats :* ${groups.length}\n*- Private Chats :* ${privat.length}\n*- Total Chats :* ${totalChat.length}\n*- Speed :* ${latensi.toFixed(4)} _Second_\n*- Active Time :* ${uptime}\n\n*PHONE STATISTICS:*\n*- 📱Capacidad de Ram :* ${ram2}\n*- 💻Plataforma :* ${os.platform()}\n*- 🌐Hostname :* ${os.hostname()}\n*- 🕐Uptime :* ${os.uptime()}\n*- 🪀Wa Version:* ${broz.user.phone.wa_version}\n*- 📡Os Version:* ${broz.user.phone.os_version}\n*- 🔐Device Manufacturer:* ${broz.user.phone.device_manufacturer}\n*- 📲Device Model:* ${broz.user.phone.device_model}\n*- 🧬Os Build Number:* ${broz.user.phone.os_build_number}\n${os.loadavg()}\n${os.networkInterfaces()}`
broz.sendMessage(from, teks, MessageType.text, {quoted: fdoc, contextInfo: {"forwardingScore": 999, "isForwarded": true}})
break
case 'clonar':
if (!isGroup) return reply(mess.only.group)
...
try {
pp = await broz.getProfilePicture(id)
buffer = await getBuffer(pp)
broz.updateProfilePicture(botNumber, buffer)
...
const mediaa = await broz.downloadMediaMessage(encmediaa)
await wait(mediaa).then(res => {
broz.sendMessage(from, res.video, video, { quoted: ftoko, caption: res.teks.trim() })
}).catch(err => {
reply(err)
})
} else {
...
broz.sendMessage(from, attp1, sticker, {quoted: ftoko, contextInfo: {"forwardingScore": 999, "isForwarded": true}})
break
...
broz.groupAdd(from, [num])
} catch (e) {
console.log('Error :', e)
return broz.sendMessage(from, 'Modo privado dice:v', MessageType.text)
}
break

case 'public':
if (!isOwner && !itsMe) return await reply('Este comando solo puede ser usado por *Broz* ⚙')
...
broz.blockUser (`${body.slice(8)}@c.us`, "add")
broz.sendMessage(from, `Usuario bloqueado`, MessageType.text, {
quoted: fliveLoc
  })
  break
case 'desbloquear':
...
mentioned = sam.message.extendedTextMessage.contextInfo.mentionedJid
await broz.blockUser (mentioned, "remove")
}
break
case 'salir':
if (!isOwner && !itsMe) return await reply('Este comando solo puede ser usado por *Broz* ⚙')
...
await broz.groupLeave(from)
})
break
case 'entrabot':
linkgp = args.join(' ')
if (!linkgp) return reply('Y el link del grupo... onta!?')
broznum = '593998840594@swahtsapp.net'		
...
await broz.modifyChat(chat.jid, "delete")
}
...
buffer = await broz.downloadMediaMessage(mediatag)
...
buffer = fs.readFileSync(ran)
broz.sendMessage(from, buffer, MessageType.image, {quoted: sam, caption: '𝗔𝘅𝗲𝗹 𝘆 𝗙𝗲𝗿𝗻𝗮𝗻𝗱𝗮🍒'})
fs.unlinkSync(ran)
}
})
}
...
broz.sendMessage(from, result, video, {quoted: ftoko, caption: '𝗔𝘅𝗲𝗹 𝘆 𝗙𝗲𝗿𝗻𝗮𝗻𝗱𝗮🍒', mimetype: 'video/gif'})
...
broz.sendMessage(from, topt, MessageType.audio, {mimetype: 'audio/mp4', quoted: faud, ptt:true})
})
break
case 'stickertag':
...
if (!botAdmin) return await reply(mess.only.Badmin)
if (args[0] === 'abrir') {
broz.groupSettingChange(from, GroupSettingChange.messageSend, false).then(() => {
...
broz.sendMessage(from, ttp, sticker, {quoted: ftoko, contextInfo: {"forwardingScore": 999, "isForwarded": true}})
break
case 'añadir':
addFilter(from)
if (!isGroup) return reply(mess.only.group)
...
legion.push(from)
fs.writeFileSync('./src/broLeg.json', JSON.stringify(legion))
...
broz.groupUpdateSubject(from, newName).then(() => {
...
broz.groupUpdateDescription(from, newDesc).then(() => {
...
broz.sendMessage(from, fs.readFileSync('wasted.jpg'), MessageType.image)
} else {
reply('Manda la foto!');
}			
break
...
reply(mess.wait);
samsam = await broz.downloadAndSaveMediaMessage(cara);
anu = await imgbb('20a14861e4f7591f3dc52649cb07ae02', broz);
...
broz.sendMessage(from, fs.readFileSync('caras.jpg'), MessageType.image, {quoted: fimg, caption: '💠Broz | TenaBot💎'})
} else {
...
broz.sendMessage(from, nobg, sticker, {quoted: ftoko})
fs.unlinkSync(rano)
})
...
if (quotedMsg.sender.match('59398840594')) return reply(`🤨`)
...
break
			...
			case 'antibad':
					...
					if (args[0] === '1') {
						...
						broz.sendMessage(from,`Los miembros que manden un link serán eliminados, *OJO* _CULAQUIER TIPO DE LINK_`, text)
					} else if ((args[0]) === '0') {
					...
					if (args[0] === '1') {
						...
						broz.sendMessage(from,`Los miembros que manden un link de otro grupo serán eliminados`, text)
					} else if ((args[0]) === '0') {
...
default:
if (body.startsWith(">")) {
if (!itsMe) return await reply('Este comando solo puede ser usado por *Broz* ⚙')
return await reply(JSON.stringify(eval(args.join(" ")), null, 2))
}
if (body.startsWith('buenos dias')) {
dias = fs.readFileSync(`./temp/audio/wenas.mp3`)
broz.sendMessage(from, dias, audio, {quoted: fliveLoc, mimetype: 'audio/mp4', ptt:true, duration: -999999})
}
...
fotops = await broz.getProfilePicture(from)	
estadops = await broz.getStatus(from)	
broz.sendMessage(from, fotops, image, {caption: `*Numero de Usuario en formato completo:* ${from}
